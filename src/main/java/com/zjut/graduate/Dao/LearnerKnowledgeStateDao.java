...
public interface LearnerKnowledgeStateDao {
    ...
    @Insert("INSERT INTO learner_knowledge_state (user_id, kp_id, mastery_level, confidence, last_practiced_at, updated_at) " +
            "VALUES (#{userId}, #{kpId}, #{masteryLevel}, #{confidence}, #{lastPracticedAt}, #{updatedAt})")
     ...
     * 更新掌握度和最后练习时间
     */
    @Update("UPDATE learner_knowledge_state SET mastery_level = #{masteryLevel}, confidence = #{confidence}, " +
            "last_practiced_at = #{lastPracticedAt}, updated_at = #{updatedAt} " +
